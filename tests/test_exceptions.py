...
import pytest
from app.polygon_geometry import check_polygon_intersection, get_polygon_union, MethodInputError, InvalidGeoJson
...
def test_exception_on_invalid_input():
    ## Test get_polygon_union() for raise(MethodInputError) on invalid method input (should be 2+ poly's) 
    l_polys = [POLY_GOOD]
    with pytest.raises(MethodInputError):
        result = get_polygon_union(l_polys) 
